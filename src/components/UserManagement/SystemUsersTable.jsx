import React from "react";
import { DataTable } from "../UI";
import SystemUsersRoleBadge from "./SystemUsersRoleBadge";
import SystemUsersActionGroup from "./SystemUsersActionGroup";
import SystemUsersPasswordControl from "./SystemUsersPasswordControl";

const getSystemUserRowKey = (row) => `system:${String(row?.id)}`;

export default function SystemUsersTable({
  users = [],
  isTogglingActive = false,
  isResettingPassword = false,
  isDeleting = false,
  onToggleActive,
  onResetPassword,
  onEdit,
  onDelete,
  currentUserId,
}) {
  const renderGuardianManagedNotice = () => (
    <span className="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
      Managed in Guardians tab
    </span>
  );

  const columns = [
    {
      key: "username",
      label: "Username",
      nowrap: true,
      headerClassName: "min-w-[14rem]",
      cellClassName: "min-w-[14rem]",
      render: (value) => (
        <div className="font-medium text-gray-900 dark:text-gray-100">{value}</div>
      ),
    },
    {
      key: "role_name",
      label: "Role",
      nowrap: true,
      headerClassName: "min-w-[11rem]",
      cellClassName: "min-w-[11rem]",
      render: (_value, row) => <SystemUsersRoleBadge user={row} />,
    },
    {
      key: "password_control",
      label: "Password",
      nowrap: true,
      headerClassName: "min-w-[10rem]",
      cellClassName: "min-w-[10rem]",
      render: (_value, row) => (
        row?.is_guardian_account ? (
          renderGuardianManagedNotice()
        ) : (
          <SystemUsersPasswordControl
            onResetPassword={() => onResetPassword?.(row)}
            disabled={isResettingPassword}
          />
        )
      ),
    },
    {
      key: "clinic_name",
      label: "Clinic",
      headerClassName: "min-w-[12rem]",
      cellClassName: "max-w-[14rem]",
      render: (value) =>
        value ? (
          <span className="text-gray-700 dark:text-gray-200 whitespace-normal break-words">
            {value}
          </span>
        ) : (
          <span className="text-gray-400 italic">N/A</span>
        ),
    },
    {
      key: "contact",
      label: "Contact",
      headerClassName: "min-w-[12rem]",
      cellClassName: "max-w-[14rem] break-all",
      render: (value) =>
        value ? (
          <span className="text-gray-700 dark:text-gray-200 break-all">{value}</span>
        ) : (
          <span className="text-gray-400 italic">N/A</span>
        ),
    },
    {
      key: "is_active",
      label: "Status",
      nowrap: true,
      headerClassName: "min-w-[8rem]",
      cellClassName: "min-w-[8rem]",
      render: (value) =>
        value ? (
          <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-sm bg-[var(--theme-success-bg)] text-[var(--theme-success)]">
            Active
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-sm bg-[var(--color-bg-tertiary)] text-[var(--color-text-primary)]">
            Disabled
          </span>
        ),
    },
  ];

  return (
    <DataTable
      data={users}
      columns={columns}
      actions={(row) => (
        row?.is_guardian_account ? (
          renderGuardianManagedNotice()
        ) : (
          <SystemUsersActionGroup
            user={row}
            isTogglingActive={isTogglingActive}
            isDeleting={isDeleting}
            onToggleActive={onToggleActive}
            onEdit={onEdit}
            onDelete={onDelete}
            currentUserId={currentUserId}
          />
        )
      )}
      getRowKey={getSystemUserRowKey}
      actionsHeaderClassName="min-w-[14rem]"
      actionsCellClassName="min-w-[14rem]"
      emptyMessage="No users found."
      emptyIcon={<span className="text-4xl">🛡️</span>}
    />
  );
}
